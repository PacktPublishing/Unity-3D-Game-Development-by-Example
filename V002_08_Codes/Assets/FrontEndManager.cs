	...
	void OnGUI()
	{
		...
		if (null != mySkin)
		{
			...
			if (ShowMenu)
			{
				...
				GUI.Box (new Rect(0, 0, Screen.width, 410), "Meltdown Madness!", "MenuBox");
			...
			else
{
			...
			{
				GUI.skin = mySkin;
				GUI.Box (new Rect(2, 0, Screen.width-4, 410), "Meltdown Madness!", "MenuBox");
				
				GUI.BeginGroup(new Rect(7, 20, Screen.width - 14, 390));
					
					GUI.Box (new Rect(0, 146, Screen.width - 14, 230), "High Scores");
					...
					if (null != CurrentScores)
					{
						if ((null != CurrentScores.Scores) && (CurrentScores.Scores.Count > 0))
						{
							...
							{
								GUI.Label(new Rect(10, 168 + (i * 24), Screen.width - 24, 20), CurrentScores.Scores[i].Initials + "    " + CurrentScores.Scores[i].Score.ToString ());
							}
						}
					}
				GUI.EndGroup();
				GUI.Box (new Rect((Screen.width/2) - 502, 425, Screen.width - 20, 48),"", "ReactorConsole");
				if (GUI.Button (new Rect((Screen.width/2) - 500, 430, Screen.width - 24, 40), "Main Menu", "ReactorStatusVenting"))
				{
					ShowingScores = false;
				}
			}
			...
			{
				GUI.Box (new Rect(2, 0, Screen.width - 14, 410), "Meltdown Madness!");
				
				GUI.BeginGroup(new Rect(7, 20, Screen.width - 24, 390));
					
					GUI.Box (new Rect(0, 146, Screen.width - 24, 230), "High Scores");
					HighScores CurrentScores = ReturnScoresList();
					if (null != CurrentScores)
					{
						if ((null != CurrentScores.Scores) && (CurrentScores.Scores.Count > 0))
				...
				GUI.EndGroup();
				GUI.Box (new Rect((Screen.width/2) - 502, 425, Screen.width - 20, 48), "");
				if (GUI.Button (new Rect((Screen.width/2) - 500, 430, Screen.width - 24, 40), "Main Menu"))
				{
					ShowingScores = false;
				}
			}
